...
def F1Score(metrics):
    ...
    #metrics_per = metrics[metrics["Target_count"] != 0].copy()
    metrics_per = metrics.copy()
    metrics_per[metrics_per['Target_count']==0] = 1
    tot_perc_diff = (metrics_per["Predicted_count"] -
                         metrics_per["Target_count"])/(metrics_per["Target_count"])
