...
if __name__ == "__main__":
    ...
    f1_summary = pd.DataFrame(columns=["F1_test"])

    model_train_list = ['fine_tuning', 'few_shot']
    model_train = model_train_list[0]
    if isinstance(model_train, list):
        model_train = '/'.join(model_train)
        model_train_csv = '_'.join(model_train.split('/'))
    else:
        model_train_csv = model_train

    models_path = '../model_results/{}/green/'.format(model_train)
    ...
    for model_name in model_names:
        ...
        if os.path.isfile(model_path):
            print('analyzing {}'.format(model_path))
        else:
            print('model not found skipping')
            continue

        if 'summary' in os.listdir(save_metrics_path):
            for f_name in os.listdir(save_metrics_path + 'summary'):
                if 'summary' in f_name:
                    summary_name = f_name
            summary_csv = pd.read_csv(save_metrics_path + 'summary/{}'.format(summary_name))
            f1_summary.loc['{}'.format(model_name)] = [summary_csv['F1_score'].values[0]]
            f1_summary.to_csv('f1_summary_{}.csv'.format(model_train_csv))
            print('already summay in this path....skipping')
            continue
        ...
        if 'few_shot' in model_train:
            if 'rs' in model_name:
                ...
                val_images_path = str(FewShot.as_posix().replace('/evaluation', '')) + '/images_{}_{}'.format(
                    num_samples, rs)
                val_masks_path = str(FewShot.as_posix().replace('/evaluation', '')) + '/masks_{}_{}'.format(num_samples,
                                                                                                            rs)
                batch_size = 4
        ...
        f1_test = metric_reports_th_sweep(args, Path(val_images_path), Path(val_masks_path),
                                Path(test_images_path), Path(test_masks_path), model_path,
        ...
        f1_summary.loc['{}'.format(model_name)] = [f1_test['F1_score'].values[0]]
        f1_summary.to_csv('f1_summary_{}.csv'.format(model_train_csv))
        print(f1_summary)
